...
class RegimeController {
    ...
    public function associerRegimeToDossier($idDossier, $idRegime) {
        try {
            $pdo = config::getConnexion();
            $query = $pdo->prepare("UPDATE dossier_medical SET id_regime = :id_regime WHERE id_dossier = :id_dossier");
    ...
    public function dissocierRegimeFromDossier($idDossier, $idRegime) {
        try {
            $pdo = config::getConnexion();
            $query = $pdo->prepare("UPDATE dossier_medical SET id_regime = NULL 
    ...
    public function countRegimesForDossier($idDossier) {
        try {
            $pdo = config::getConnexion();
            $query = $pdo->prepare("SELECT CASE WHEN id_regime IS NULL THEN 0 ELSE 1 END as total
                                   FROM dossier_medical WHERE id_dossier = :id_dossier");
            $query->execute([':id_dossier' => $idDossier]);
