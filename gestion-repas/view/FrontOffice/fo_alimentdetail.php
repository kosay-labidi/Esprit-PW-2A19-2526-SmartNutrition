<?php
// view/FrontOffice/alimentdetail.php  — FRONT OFFICE
...
$ns  = nutriScore($a);
$mp  = macroPercents($a);
$svg = alimentSVG($a['nom'], $a['type'], $c, 90);
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <style>
        ...
        * { font-family:'Lato',sans-serif; box-sizing:border-box; margin:0; padding:0; }
        ...
        .stat-val { font-size:26px;font-weight:700;color:var(--vert);font-family:'Cormorant Garamond',serif; }
...
<nav class="navbar text-white sticky top-0 z-50 shadow-xl">
    <div class="max-w-screen-xl mx-auto px-8 py-4 flex items-center justify-between">
        ...
        <ul class="flex items-center gap-7 text-sm font-medium">
            ...
            <li><a href="#"               class="hover:text-[#a78bfa] transition-colors">Mes repas</a></li>
...
<div class="max-w-screen-xl mx-auto px-8 pt-5">
    <nav style="font-size:13px;color:#9ca3af;display:flex;align-items:center;gap:6px;">
        <a href="alimentlist.php" style="color:var(--vert);text-decoration:none;font-weight:600;">Aliments</a>
        <span>/</span>
...
<!-- HERO ALIMENT -->
<div style="background:linear-gradient(135deg,<?= $c['bg'] ?> 0%,white 70%);border-bottom:1px solid <?= $c['stroke'] ?>33;margin-top:10px;">
        ...
        <div style="display:flex;align-items:center;gap:28px;flex-wrap:wrap;">
            ...
            <div style="flex:1;min-width:260px;">
                <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:10px;">
                    ...
                    <?php if (!empty($a['label_ecologique'])): ?><span class="badge" style="background:#e8f0e9;color:var(--vert);">🌱 <?= htmlspecialchars($a['label_ecologique']) ?></span><?php endif; ?>
                    <?php if (!empty($a['origine'])): ?><span class="badge" style="background:#f4ede4;color:var(--vert);">📍 <?= htmlspecialchars($a['origine']) ?></span><?php endif; ?>
                </div>
                <h1 class="hf" style="font-size:54px;color:var(--vert);line-height:1;margin:0 0 16px;"><?= htmlspecialchars($a['nom']) ?></h1>

                <!-- Stats rapides -->
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(90px,1fr));gap:10px;max-width:600px;">
                    <?php
                    $stats = [
            ...
            <div style="text-align:center;flex-shrink:0;">
                <div style="width:64px;height:64px;border-radius:14px;background:<?= $ns['bg'] ?>;color:<?= $ns['color'] ?>;display:flex;align-items:center;justify-content:center;font-size:32px;font-weight:700;margin:0 auto 6px;" class="hf"><?= $ns['grade'] ?></div>
...
<div class="max-w-screen-xl mx-auto px-8 py-10">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
        ...
        <div style="display:flex;flex-direction:column;gap:20px;">
            ...
            <!-- Valeurs nutritionnelles + donut -->
            <div class="card">
                <h2 class="hf" style="font-size:24px;color:var(--vert);margin-bottom:18px;">Valeurs nutritionnelles <span style="font-size:14px;font-weight:400;color:#9ca3af;font-family:'Lato',sans-serif;">pour 100g</span></h2>
                ...
                <div style="display:flex;align-items:center;gap:24px;margin-bottom:22px;">
                    <div class="donut-wrap">
                        ...
                        <div class="donut-center">
                            <p style="font-size:20px;font-weight:700;color:var(--vert);font-family:'Cormorant Garamond',serif;margin:0;"><?= number_format($a['calories'],0) ?></p>
                            <p style="font-size:10px;color:#9ca3af;margin:0;">kcal</p>
                        </div>
                    </div>
                    <div style="flex:1;">
                        <?php
                        $legend = [['#60a5fa','Protéines',$mp['prot']],['#c9a44a','Glucides',$mp['gluc']],['#a78bfa','Lipides',$mp['lip']]];
                ...
                <!-- Barres détaillées — toutes les valeurs -->
                <?php
                $macros = [
                ...
                <div class="mrow" style="<?= $indent ?>">
                    ...
                    <span style="font-size:12px;font-weight:700;color:var(--vert);width:70px;text-align:right;flex-shrink:0;"><?= number_format($val,1) ?> <?= $unit ?></span>
                </div>
                <?php endforeach; ?>

                <?php if (!empty($a['vitamines'])): ?>
                <div style="margin-top:16px;padding-top:16px;border-top:1px solid #f4ede4;">
                    <p style="font-size:12px;color:#9ca3af;margin-bottom:4px;text-transform:uppercase;letter-spacing:.05em;font-weight:600;">Vitamines</p>
            ...
            <div class="card" style="border-color:#c09090;background:#fdf5f5;">
                ...
                <div style="display:flex;flex-wrap:wrap;gap:8px;">
                    ...
                    <span style="background:#faeaea;color:#8a2020;padding:5px 14px;border-radius:99px;font-size:13px;font-weight:500;"><?= htmlspecialchars(trim($al)) ?></span>
                    <?php endforeach; ?>
                </div>
            </div>
            <?php endif; ?>
        ...
        <div style="display:flex;flex-direction:column;gap:20px;">
            ...
            <div class="card">
                <h2 class="hf" style="font-size:24px;color:var(--vert);margin-bottom:18px;display:flex;align-items:center;gap:8px;">
                    <i class="fas fa-leaf" style="font-size:16px;color:#1a372f;"></i> Impact écologique
                </h2>
                ...
                <div style="background:<?= $co2['bg'] ?>;border-radius:14px;padding:18px;margin-bottom:14px;">
                    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;">
                        <div>
                            <p style="font-size:28px;font-weight:700;color:<?= $co2['color'] ?>;font-family:'Cormorant Garamond',serif;margin:0;"><?= number_format($a['co2'],2) ?> kg CO₂eq</p>
                            <p style="font-size:12px;color:<?= $co2['color'] ?>;opacity:.75;margin:3px 0 0;">par kg d'aliment</p>
                        </div>
                        <span style="background:<?= $co2['color'] ?>;color:white;padding:6px 16px;border-radius:99px;font-size:13px;font-weight:600;"><?= $co2['label'] ?></span>
                    </div>
            ...
            <div class="card">
                ...
                <div style="display:flex;align-items:baseline;gap:6px;">
                    <span style="font-size:40px;font-weight:700;color:var(--vert);font-family:'Cormorant Garamond',serif;"><?= number_format($a['prix'],2) ?></span>
            ...
            <div class="card" style="background:linear-gradient(135deg,<?= $c['bg'] ?> 0%,white 100%);border-color:<?= $c['stroke'] ?>44;">
                ...
                <p style="font-size:13px;color:#6b7280;margin-bottom:16px;line-height:1.6;">
                    Ajoutez <strong><?= htmlspecialchars($a['nom']) ?></strong> à un repas pour suivre votre apport nutritionnel et votre impact environnemental.
                </p>
                <a href="#" style="display:inline-flex;align-items:center;gap:8px;background:var(--vert);color:white;padding:10px 22px;border-radius:99px;font-size:13px;font-weight:600;text-decoration:none;">
            ...
            <a href="alimentlist.php"
               ...
               onmouseout="this.style.borderColor='#d0c8be';this.style.color='#4b5563'">
                <i class="fas fa-arrow-left" style="font-size:12px;"></i> Retour à la liste
            </a>
        </div>
    </div>
</div>
...
<script>
// Curseur
(function(){const c=document.getElementById('cur'),t=document.getElementById('curt');let mx=0,my=0,tx=0,ty=0;document.addEventListener('mousemove',e=>{mx=e.clientX;my=e.clientY;c.style.left=mx+'px';c.style.top=my+'px';});(function l(){tx+=(mx-tx)*.12;ty+=(my-ty)*.12;t.style.left=tx+'px';t.style.top=ty+'px';requestAnimationFrame(l);})();document.querySelectorAll('a,button').forEach(el=>{el.addEventListener('mouseenter',()=>c.classList.add('h'));el.addEventListener('mouseleave',()=>c.classList.remove('h'));});})();

// Donut Chart
new Chart(document.getElementById('donut').getContext('2d'),{
    type:'doughnut',
    data:{labels:['Protéines','Glucides','Lipides'],datasets:[{data:[<?= $mp['prot'] ?>,<?= $mp['gluc'] ?>,<?= $mp['lip'] ?>],backgroundColor:['#60a5fa','#c9a44a','#a78bfa'],borderWidth:0,hoverOffset:4}]},
    options:{cutout:'68%',plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>` ${ctx.label} : ${ctx.raw}%`}}},animation:{duration:900}}
});
</script>
</body>
</html>
